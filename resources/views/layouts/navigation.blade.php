<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(auth()->user()->isAdmin() || auth()->user()->isResepsionis() || auth()->user()->isDokter() || auth()->user()->isPerawat())
                    ...
                    @if(auth()->user()->isResepsionis() || auth()->user()->isAdmin())
                        <x-nav-link :href="route('queues.monitor')" :active="request()->routeIs('queues.monitor')">
                            {{ __('Monitor Antrean') }}
                        </x-nav-link>
                    @endif

                    @if(auth()->user()->isPasien())
                        <x-nav-link :href="route('queues.book')" :active="request()->routeIs('queues.book')">
                            {{ __('Ambil Antrean') }}
                        </x-nav-link>
                    @endif
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                ...
                <button id="theme-toggle" type="button" class="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none rounded-lg text-sm p-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 transition">
                    ...
                    <svg id="theme-toggle-light-icon" class="hidden w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zm-5.05-1.707l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm-2.12-10.607a1 1 0 011.414 0l.706.707a1 1 0 11-1.414 1.414l-.707-.707a1 1 0 010-1.414zM4 11a1 1 0 100-2H3a1 1 0 100 2h1z" fill-rule="evenodd" clip-rule="evenodd"></path>
                    </svg>
                </button>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->isResepsionis() || auth()->user()->isAdmin())
                <x-responsive-nav-link :href="route('queues.monitor')" :active="request()->routeIs('queues.monitor')">
                    {{ __('Monitor Antrean') }}
                </x-responsive-nav-link>
            @endif

            @if(auth()->user()->isPasien())
                <x-responsive-nav-link :href="route('queues.book')" :active="request()->routeIs('queues.book')">
                    {{ __('Ambil Antrean') }}
                </x-responsive-nav-link>
            @endif

            @if(auth()->user()->isApoteker() || auth()->user()->isAdmin())
                <x-responsive-nav-link :href="route('pharmacy.prescriptions')" :active="request()->routeIs('pharmacy.prescriptions')">
                    {{ __('Antrean Resep') }}
                </x-responsive-nav-link>
